...
# A working login and dashboard for user
...
#Final Programming Assignment - Dashboard View that is accessible after login 
@login_required
def dashboard(request):
     # Get full name if available; otherwise, use username
    doctor_name = request.user.get_full_name() or request.user.username
    return render(request, 'basic/dashboard.html', {'doctor_name': doctor_name})

#Final Programming Assignment - Logout View that will redirect to login page
...
def test_introduction(request):
    return render(request, "basic/test_introduction.html")
